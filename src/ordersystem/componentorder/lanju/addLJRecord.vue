<template>
  <div class="bank-details">
    ...
    <div class="bank-details-msg">
      ...
      <div class="msg">
        <div @click="showBank = true"><span class="left">汇入银行</span><img class="right more-slet" src="../../assetsorder/more.png" alt=""><input readonly="readonly" placeholder="填写汇入玉兰账号所属银行" class="right bank-input" v-model="bankDetails.yulanBank"></div>
        <div @click="showHkHistory = true"><span class="left">付款公司(人)</span><img class="right more-slet" src="../../assetsorder/more.png" alt=""><input placeholder="填写付款公司(人)" class="right bank-input" v-model="bankDetails.payerName"></div>
        <div><span class="left">付款银行账号</span><img class="right more-slet" src="../../assetsorder/more.png" alt=""><input placeholder="填写付款银行账号" class="right bank-input" v-model="bankDetails.payerAccount"></div>
        <div><span class="left">付款金额</span><img class="right more-slet" src="../../assetsorder/more.png" alt=""><input placeholder="填写付款金额" class="right bank-input" v-model.number="bankDetails.payAmount" type="number" @input="oninput"></div>
        <div @click="showselTime = true"><span class="left">付款日期</span><img class="right more-slet" src="../../assetsorder/more.png" alt=""><input readonly="readonly" placeholder="请选择日期" class="right bank-input" v-model="bankDetails.payDate"></div>
        <div><span class="left">备注</span><img class="right more-slet" src="../../assetsorder/more.png" alt=""><input class="right bank-input" placeholder="填写备注" v-model="bankDetails.memo"></div>
      </div>
      <div class="fkpz">
        <div class="fkpz-title">付款凭证</div>
        <van-uploader :after-read="onRead" :before-read="beforeRead"  accept="image/*" :preview-size="100" :max-count="1" :max-size="2*1024*1024">
          <img v-if="bankDetails.imgUrl" class="bank-img head-img" :src="imgSrc" ref="goodsImg" style="width: 150px;height: 150px;"/>
    ...
    <van-popup v-model="showHkHistory" class="showHkHistory">
      <input class="fill-conpony" placeholder="填写付款公司(人)" type="text" v-model="bankDetails.payerName">
      ...
      <van-radio-group v-model="componyBank">
        <van-cell-group>
          ...
          <van-cell :title="item" clickable @click="onConfirmHkHistory(item,index)">
...
<script>
  ...
  import { Popup,Dialog ,Toast, Collapse, CollapseItem ,DatetimePicker,Uploader ,Button ,RadioGroup, Radio,Cell,CellGroup} from 'vant';
    export default {
      name: "addLJRecord",
      components:{
        top,
      ...
      data(){
          return{
            ...
            showPic: false,
            showBank: false,
            bank: ['中信银行', '中国工商银行9761', '中国工商银行8881', '中国邮政储蓄'],
            hKHistory: [],
            hKHistoryRecord:[],
            componyBank:'',
            showHkHistory: false,
      ...
      // //  创建凭证
        toEditBank () {
          if (!(this.bankDetails.yulanBank && this.bankDetails.payerName && this.bankDetails.payerAccount && this.bankDetails.payDate && this.bankDetails.payAmount)) {
            Toast({
              message:"请完善信息",
              duration:2000
            })
      ...
      //  获取汇款账号历史
        ...
        confirmTime (value) {
          ...
          this.bankDetails.payDate = current_year + '-' + current_month + '-' + current_date
        ...
        oninput(e) {
          ...
          console.log(e.target.value, this.bankDetails.payAmount)
        },
      },
      created () {
        // this.initDetails()
