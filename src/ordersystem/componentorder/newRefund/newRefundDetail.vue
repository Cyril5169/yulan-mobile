    ...
    <div class="lanju-details-msg">
      ...
      <div class="msg">
        <div style="margin-top:8px;">
          <span class="left">单据号：</span>
          <span class="right">{{checkTable.ID}}</span>
        </div>
        <div>
          <span class="left">提交时间：</span>
          <span class="right">{{checkTable.CREATE_TS|dateTrans}}</span>
        </div>
        <div>
          <span class="left">单据状态:</span>
          <span class="right">{{checkTable.STATE|statusTrans}}</span>
        </div>
      </div>
      <div class="msg">
        <div style="margin-top:8px;">
          <span class="left">提货单号：</span>
          <span class="right">{{checkTable.SALE_NO}}</span>
        </div>
        <div>
          <span class="left">B2B订单号：</span>
          <span class="right">{{checkTable.ORDER_NO}}</span>
        </div>
        <div>
          <span class="left">ERP订单号：</span>
          <span class="right">{{checkTable.CONTRACT_NO}}</span>
        </div>
        <div>
          <span class="left">物流单号：</span>
          <span class="right">{{checkTable.TRANS_ID}}</span>
        </div>
      </div>
      <div class="msg">
        <div style="margin-top:8px;">
          <span class="left">客户代码：</span>
          <span class="right">{{checkTable.CID}}</span>
        </div>
        <div>
          <span class="left">客户名称：</span>
          <span class="right">{{checkTable.CNAME}}</span>
        </div>
        <div>
          <span class="left">联系人：</span>
          <span class="right">{{checkTable.CONTACT_MAN}}</span>
        </div>
        <div>
          <span class="left">联系电话：</span>
          <span class="right">{{checkTable.CONTACT_PHONE}}</span>
        </div>
      </div>
      <div class="msg">
        <div style="margin-top:8px;">
          <span class="left">产品型号：</span>
          <span class="right">{{checkTable.ITEM_NO}}</span>
        </div>
        <div>
          <span class="left">产品名称：</span>
          <span class="right">{{productVersionTrans(checkTable.PRODUCTION_VERSION)}}</span>
        </div>
        <div>
          <span class="left">单位：</span>
          <span class="right">{{checkTable.UNIT}}</span>
        </div>
        <div>
          <span class="left">数量：</span>
          <span class="right">{{checkTable.QTY}}</span>
        </div>
        <div>
      ...
      <div class="msg" v-if="checkTable.STATE!='SUBMITTED'">
        <div style="margin-top:8px;">
          ...
          <span class="right">{{checkTable.RETURN_TYPE}}</span>
        </div>
        <div>
        ...
        <div v-if="checkTable.RETURN_TYPE=='玉兰取货'">
          <span class="left">备注信息：</span>
          <span class="right">我公司已安排物流公司上门取货</span>
        </div>
        <div v-if="checkTable.RETURN_TYPE=='玉兰取货'">
          <span class="left">备注信息：</span>
          <span class="right">请保持电话畅通</span>
        </div>
        <div v-if="checkTable.RETURN_TYPE=='客户邮寄'">
          <span class="left">备注信息：</span>
          <span class="right">请您在快递单上备注提货单号</span>
        </div>
        <div v-if="checkTable.RETURN_TYPE=='客户邮寄'">
          <span class="left">退货或寄样信息：</span>
          <span
            v-if="checkTable.RETURN_ADDRESS.length<=35"
            class="right"
          >{{checkTable.RETURN_ADDRESS}}</span>
          <!-- <span
            v-if="getRealStrLength(checkTable.RETURN_ADDRESS)<=35"
            class="right"
          >{{checkTable.RETURN_ADDRESS}}</span> -->
        </div>
        ...
        <div v-if="checkTable.RETURN_TYPE=='客户邮寄'">
          <span class="left">邮寄备注信息：</span>
          <span class="right">您的提货单号：{{checkTable.SALE_NO}}</span>
        </div>
      </div>
      ...
      <div v-if="checkTable.STATE=='CUSTOMERAFFIRM'||checkTable.STATE=='APPROVED'">
        <div class="msg" v-for="(item,index) of checkDetailTable" :key="index">
          <div style="margin-top:8px;">
            <span class="left">第{{index+1}}条处理结果：</span>
          ...
          <div>
            <span class="left">数量：</span>
            <span class="right">{{item.P_QTY}}</span>
          </div>
          <div>
            <span class="left">金额：</span>
            <span class="right">{{item.P_MONEY}}</span>
          </div>
          <div>
          ...
          <div>
            ...
            <span class="right">{{item.P_RESULT}}</span>
          </div>
        </div>
      </div>
      <div class="msg" v-if="checkTable.STATE=='CUSTOMERAFFIRM'||checkTable.STATE=='APPROVED'">
        <div style="margin-top:8px;">
          <span class="left">金额小写：</span>
          <span class="right">{{checkTable.TOTALMONEY}}</span>
        </div>
        <div>
          <span class="left">金额大写：</span>
          <span class="right">{{totalMoneyUpper}}</span>
        </div>
      </div>
    </div>
    <div class="edit-bank">
      ...
      <span
        v-if="checkTable.STATE=='CUSTOMERAFFIRM'"
        class="edit-bank-xg"
        @click="CustomerApproved()"
      >同意</span>
    </div>
    <van-popup v-model="showNote" position="bottom">
      <div style="margin:4px 0px 4px 4px;text-align:left;font-size:20px;color:red">注意事项：</div>
      <div style="margin:4px 0px 4px 4px;text-align:left;">
        1.若您未在我公司对您的《退货/赔偿申请书》提交处理意见之日起15日内确认、提出异议的，则视为放弃赔偿权利；
        <br />2.玉兰公司支付的退货金额，仅限于本《退货/赔偿申请书》的金额，不承担其他费用；
        <br />3.请您仔细阅读本《退货/赔偿申请书》相关信息，一旦确认，视为同意我公司的处理方案。
        <br />
        <br />公司名称：广东玉兰集团股份有限公司
        <br />地址：东莞市莞城莞龙路段狮龙路莞城科技园内
        <br />电话:0769-23321708&emsp;&emsp;邮政编码:523119
        <br />邮箱：yulan315@yulangroup.cn
        <br />
      </div>
    </van-popup>
  </div>
    ...
    checkDetails() {
      ...
      GetCompensationById(data).then(res => {
        if (res.count > 0) {
          ...
          if (
            this.checkTable.STATE == "CUSTOMERAFFIRM" ||
